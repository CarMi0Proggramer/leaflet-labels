    ...
    if (this.textOverflow != "visible") {
      if (
        ...
        this.measureText(text).width +
          (text.length - 1 + nbspace) * letterPadding
      ) {
        var overflow =
          this.textOverflow == "ellipsis" ? "\u2026" : this.textOverflow || "";
        ...
        do {
          ...
          text &&
          d <
            this.measureText(text + overflow).width +
              (text.length + dt + nbspace) * letterPadding
        );
        text += overflow;
      }
    }
    ...
    switch (this.textJustify || this.textAlign) {
      ...
      case "right": {
        ...
        if (this.textJustify) {
          start = 0;
          letterPadding =
            (d - this.measureText(text).width) / (text.length - 1 + nbspace);
        }
        // Text align
        else {
          start =
